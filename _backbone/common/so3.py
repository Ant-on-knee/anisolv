...
# Path to the baked grid constants (replaces e3nn's FromS2Grid/ToS2Grid construction;
# see _GRID_MATS_PATH usage in SO3_Grid.__init__).
_GRID_MATS_PATH = Path(__file__).resolve().parents[2] / "_const" / "grid_mats.pt"
_GRID_MATS_CACHE = None
...
class SO3_Grid(torch.nn.Module):
    ...
    def __init__(
        ...
        # for lmax==mmax); the converter bakes exactly that case into _const/grid_mats.pt.
        global _GRID_MATS_CACHE
        if not (lmax == 2 and mmax == 2):
            raise NotImplementedError(
                f"standalone SO3_Grid only supports lmax==mmax==2 (baked grid); "
                f"got lmax={lmax}, mmax={mmax}"
            )
        if _GRID_MATS_CACHE is None:
            _GRID_MATS_CACHE = torch.load(_GRID_MATS_PATH, map_location="cpu", weights_only=True)
        to_grid_mat = _GRID_MATS_CACHE["to_grid_mat"].clone()
        from_grid_mat = _GRID_MATS_CACHE["from_grid_mat"].clone()
