...
class SolventEmbedding(nn.Module):
    ...
    channel), so the forward pass has no data-dependent control flow.

    Args:
        solvent_input_dim: Width of the solvent conditioning vector (SOLVENT_DIM).
    ...
    def __init__(
        ...
        # zero and does not destabilize early training. (Inference loads weights,
        # so this only matters for a from-scratch build; kept for fidelity.)
